...
### 5.12.1 WhatsApp deep link payload 规范（codex v1.1 review 补充）

WhatsApp 跳出按钮把 Step 1-3 数据序列化进 wa.me/<advisor>?text=<payload>。Payload schema：

```
v1: pandatravel | <when_text> | <group_text> | <interests_text>
    e.g.: "v1 | Apr 2027 / ~12 days | family +2 kids 6&9 | nature, food, slow"
```
...
| 字段 | 内容 | 编码 | 长度上限 |
|---|---|---|---|
| `version` | `v1` 字面量（可升级 v2 时识别） | ascii | 2 |
| `brand` | `pandatravel` 字面量（顾问看到 payload 知道来源） | ascii | 11 |
| `when_text` | "Apr 2027 / ~12 days" 这种自然语言，不是 raw json | URL-encode | 60 |
| `group_text` | "family +2 kids 6&9" 这种自然语言 | URL-encode | 60 |
| `interests_text` | "nature, food, slow" 多选 chip 拼接 | URL-encode | 80 |
| `total payload` | 含分隔符 `\|` | URL-encode | 220（WhatsApp message URL 实测限 ~280 字符 buffer） |

**隐私边界**：
- ❌ 不放 email / phone / 真名（用户还没填，不该塞 placeholder）
- ❌ 不放 user-agent / IP / referrer（即便能拿到也不能塞）
- ❌ 不放 IDs / session token / 跟踪参数（D4 透明承诺反例）
- ✅ 只放用户主动填的"行程意向"（when / group / interests）
- ✅ 顾问端看到 payload 后**手动 reply** 询问联系方式，不能自动入库

**实现要点**：
- 序列化用 `encodeURIComponent` 整个 payload string，避免 `&` / `=` / `?` 字符破坏 URL 结构
- WhatsApp 端发的是 `https://wa.me/<advisor_phone>?text=<encoded_payload>`，advisor_phone 来自 `lib/data/advisors.ts` 当前在线 advisor（v1 可只配 1 个 default）
- 长度超限时降级：interests_text → "see web form for details"
- localStorage 同步保存 payload 给"中途回来继续填"用户提供 resume 体验
- 测试用例：每个 step 1-3 状态 × 3 行 unit test 校验 payload 长度 / 编码 / version 前缀

**版本化策略**：v1 lock 上面字段。v2 改 schema（比如加 budget）必须升 version 字面量到 v2，advisor 端 parse 时按 version 路由。永远不破 v1 老链接。




### 5.13 240h 过境免签**决策工具** `app/visa-free/page.tsx` ✱ 新增

D2 + P3 差异化主入口。**不是攻略列表，是交互式决策工具**（codex review 反馈：列表形态差异化太弱）。

> **政策事实基础（lastReviewed: 2026-05-22）**
> 中国国家移民管理局 2025-11-04 公告：240h 过境免签（10 天）/ 55 国 / 65 口岸 / 24 个省级区域 / 必须前往第三国或地区。来源：[english.www.gov.cn](https://english.www.gov.cn/news/202511/04/content_WS69094ae0c6d00ca5f9a07472.html)。
> v1.1 误写为 "144h / 39 国 / 32 城" 是过期数据，v1.2 已修正。

```
1. Hero ("240 hours in China without a visa. Use them well.")

2. Decision Tool 核心交互（client component 单页 wizard，不导航）：
   Step a. Pick your passport country     [select 55 国 + search filter]
   Step b. Pick your entry port            [select 65 口岸（机场 / 陆港 / 港珠澳大桥 / 高铁 西九龙站等）]
   Step c. Pick your onward destination    [必填：第三国/地区，不能回原出发国 → 港澳台 / 泰国 / 韩国 / 日本 / 越南 / 等]
   Step d. Stay duration                   [radio: 24h 直接过境 / 240h 过境免签（10 天），根据 a+b 自动启用/禁用]
   → 即时输出：
     - "你符合 240h 过境免签 ✅ / 你不符合 ❌（一句话理由 — 比如护照国不在 55 国白单 / 出境目的地不算第三国 / 入境口岸不在 65 港名单）"
     - "你能玩的省级区域"（高亮 24 省地图 / 列表）
     - "推荐路线" 1-3 条卡片（每条引到 /itineraries/[slug]，按 entry port + region 匹配）
     - "风险 + 提示" 3-5 条（如出境必须从 a 选定的入境口岸离境 OR 同区域口岸 / 不能延长 / 不能跨区域 / 不允许工作或留学）

3. Caveats（诚实段，不藏）：
   - 时间怎么算（24 小时入境次日 0 时起算 vs 实际入境时刻 — 各口岸规则可能不同，标 lastReviewed）
   - 出境约束（必须从合规口岸 + 不能延长 + 跨省限制按 24 省具体规则）
   - 哪些活动不允许（禁止跨省工作 / 留学 / 长期居住 / 商务洽谈算不算 — 标灰色地带）
   - 哪些情况会被拒（机票回程国 = 出发国 → 不算第三国 / 重复入境冷却期 / 持有过签证记录复杂）
   - **政策刷新风险**：2024-2025 年中国持续放宽，本页 lastReviewed 时间戳显示 + 用户应自行向最新官方核实（链接到 nia.gov.cn）

4. Ready-made 240h itineraries（3-4 条预设卡 → /itineraries/visa-free-[slug]，按 entry port 分）：
   - Beijing 240h（10 天 北京-天津-河北区域）
   - Shanghai 240h（10 天 长三角：上海-杭州-苏州-南京）
   - Guangzhou 240h（10 天 珠三角：广州-深圳-珠海，含港珠澳大桥入境路径）
   - Chengdu/Chongqing 240h（10 天 川渝区域）
...
5. Quick contact（D2 决策代理关键）：
   ...
   - "你的国家不在 55 国白单？" → 跳免签政策升级订阅 / Visa apply 链接
...
```ts
type VisaFreeRule = {
  passportCountry: string;        // ISO 3166-1 alpha-2，55 国白单（参考 NIA 2025-11 公告）
  eligibleStays: ('24h' | '240h')[];   // 注：24h 是直接过境无需出关，240h 是 10 天过境免签
  eligibleEntryPorts: string[];   // port slug 数组（指 65 个口岸的 slug，含机场/陆港/海港）
  notes: { en: string; zh: string }[];   // 该国特殊规则
  source: 'NIA-2025-11-04' | string;     // 政策依据版本
  lastReviewed: string;           // YYYY-MM-DD，季度 review 一次
};

type EntryPort = {
  slug: string;
  cn: string; en: string;
  type: 'airport' | 'land-port' | 'sea-port' | 'rail-station' | 'bridge';
  applicableStays: ('24h' | '240h')[];
  province: string;               // 24 省一员
  region: 'beijing-tianjin-hebei' | 'yangtze-river-delta' | 'pearl-river-delta' | 'sichuan-chongqing' | 'other';
  exitConstraints: { en: string; zh: string };  // 出境约束（同口岸 / 同区域 / 不限）
  source: 'NIA-2025-11-04' | string;
  lastReviewed: string;
};

type ThirdDestination = {
  countryCode: string;            // ISO 3166-1 alpha-2，列举常见第三国（HK / MO / TW / TH / KR / JP / VN / SG ...）
  acceptedAsOnward: boolean;      // 是否被中国 NIA 认可为合规第三国/地区
  notes: { en: string; zh: string }[];
};
```

数据真实来源：中国国家移民管理局 2025-11-04 公告 + 持续更新。**真值不 mock**。lastReviewed 季度刷新，每次政策变化时 worker 必须更新本字段。

**差异化重点 vs 列表形态**：
- chinahighlights 没专题，我们针对 P3 做主入口
- 不是 "看清楚再决定"，是 "选完就知道能不能 / 能玩什么"，把决策成本前置到 30 秒交互
- WhatsApp 直链不藏在 modal，急客一键直达（D2 决策焦虑代理实质）
- **政策刷新优势**：chinahighlights 16 年老站静态页大概率没及时更新到 240h（spec 起草时验证 v1 也踩了这个坑），我们 lastReviewed 时间戳 + 季度 review 是结构性优势
...
components/**/*.tsx
  ...
  - skip MockBadge.tsx 实现文件本身（避免组件定义自报）

app/**/*.tsx                        ← v1.2 扩扫，codex 二审建议
  - 任何 <MockBadge .../> JSX 引用（页面级 mock 调用，之前漏扫）
```
...
**集成**：
- `package.json prebuild` 钩子，build 前自动扫（环境为 production 时启用）
- ⭐ **CI 必跑**（codex v1.1 review 强约束，v1.2 加）：GitHub Actions / 任何部署 pipeline 必须将 `pnpm prelaunch-check` 作为部署前必过 step。Pipeline 配置示例：

```yaml
# .github/workflows/deploy.yml
- name: Mock guard release gate
...
- v1.2 (2026-05-22, lead 改写, 整合 codex 二审 caveat)：
  - **§5.13 visa-free 数字硬伤修正**：v1.1 写的 144h / 39 国 / 32 城是过期数据。codex 二审现场查 gov.cn 2025-11-04 公告，**当前真值是 240h / 55 国 / 65 口岸 / 24 省级区域**。lead 用 search-router 二次核对官方 source 确认。Schema 加 lastReviewed + source 字段、加 ThirdDestination type、把 "必须不同省" 改为 "必须前往第三国/地区"。新增 EntryPort.type 区分 airport / land-port / sea-port / rail-station / bridge（含港珠澳大桥 + 西九龙站等新增口岸）
  - **§5.12.1 新增 WhatsApp deep link payload 规范**：含 schema / 字段约束（version + brand + when_text + group_text + interests_text）/ 编码（encodeURIComponent）/ 长度上限（220 字符 buffer）/ 隐私边界（不放 email/phone/真名/IDs）/ 版本化策略（v1 lock，v2 改 schema 必须升前缀）
  - **§6.5 mock guard 扩扫 + CI 必跑硬约束**：扫描范围加 `app/**/*.tsx`（v1 漏扫页面级 MockBadge 调用）+ 明确 GitHub Actions 必须跑 prelaunch-check 作为部署前 gate + 给 yaml step 示例
- 待：Phase 2.x 实施开干（不再三审）
